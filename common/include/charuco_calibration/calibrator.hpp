...
inline std::ostream& operator<<(std::ostream& s, const CalibratorParams& params)
{

  s << "squaresX: " << params.squaresX<< std::endl;
  s << "squaresY: " << params.squaresY<< std::endl;
  s << "squareLength: " << params.squareLength<< std::endl;
  s << "markerLength: " << params.markerLength<< std::endl;
  s << "dictionaryId: " << params.dictionaryId<< std::endl;
  s << "performRefinement: " << params.performRefinement<< std::endl;
  s << "drawHistoricalMarkers: " << params.drawHistoricalMarkers<< std::endl;
  s << "calibrationFlags: " << params.calibrationFlags<< std::endl;
  return (s);
}
...
struct CalibratorDetectionResult
{
	...
	bool isValid() const
	{
		return ids.size() > 0 && corners.size() > 0;
	}
