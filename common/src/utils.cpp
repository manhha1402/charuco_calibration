...
{
#define GET_PARAM(paramName)                                                                   \
	{                                                                                          \
		params->paramName = nh.param(#paramName, params->paramName);                           \
...
bool readCalibrationFlags(ros::NodeHandle& nh, int& calibrationFlags)
{
	...
	calibrationFlags = 0;
#define GET_FLAG(flag_name)                                                                              \
	{                                                                                                    \
		bool flagSet = nh.param("calibration_flags/" #flag_name, false);                                 \
...
void readCalibratorParams(ros::NodeHandle& nh, charuco_calibration::Calibrator& calibrator)
{
  ...
  calibrator.params_.performRefinement = nh.param("perform_refinement", true);
